...
#pragma omp parallel for collapse(2) private(Cb, Ab, Bb)
  for(int i=0; i<M; i+=BM)
	...
	for(int kb=0; kb<BN; kb++)
	  for(int jb=0; jb<BM; jb++)
	    Bb[kb*BM + jb] = B[(k+kb)*M + (j+jb)];

	
	for(int ib=0; ib<BM; ib++)
	  for(int jb=0; jb<BM; jb++)
	    for(int kb=0; kb<BN; kb++)
	      Cb[ib*BM + jb] += Ab[ib*BN + kb] * Bb[kb*BM + jb];
...
main(int argc, char *argv[])
{
  ...
  double N_fp = 2.0*M*M*N /* _TODO_A_ calculate N_fp as function of N, M -
			   * number of floating point operations of matrix-matrix
			   * multiplication */;
