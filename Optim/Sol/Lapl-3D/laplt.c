...
init_latparams(size_t L, size_t Sy, size_t Sx)
{
  ...
  lp.Sy = Sy;   /* The tile length along y */
  lp.Sx = Sx;   /* The tile length along x */
  lp.lx = L/Sx; /* The number of tiles along x */
  lp.ly = L/Sy; /* The number of tiles along y */
  return lp;
}
 ...
 *
 * The sites are traversed in the same order as the non-tiled version,
...
rand_links(latparams s, link **g)
{
  ...
  for(int z=0; z<L; z++)
    for(int y=0; y<L; y++)
      for(int x=0; x<L; x++) {
	int xt = x % Sx; /* Coordinates (x, y) within the tile */
	int yt = y % Sy;  
	int xs = x / Sx; /* Coordinates (x, y) of the tile */
	int ys = y / Sy;
	int xyz = xt + Sx * (yt + Sy * z);  /* Lexic. index within the tile */
	int tc = xs + ys*lx;                /* Lexic. index of the tile */
	for(int d=0; d<ND; d++) {
	  double u0 = drand48();
	  double u1 = drand48();
...
#pragma omp for collapse(2)  
  for(int y=0; y<ly; y++)
    for(int x=0; x<lx; x++) {
      /* Loop over z within tower of tiles, and then over x,y inside
	 tile
       */
      int tc = x + lx*y;              /* Lexic. index of tile */
      int tc0p = (x+1)%lx + lx*y;     /* Lexic. index of tile in +x direction */
      int tc0m = (lx+x-1)%lx + lx*y;  /* Lexic. index of tile in -x direction */
      int tcp0 = x + lx*((y+1)%ly);   /* Lexic. index of tile in +y direction */
      int tcm0 = x + lx*((y-1+ly)%ly);/* Lexic. index of tile in -y direction */
      field *qtc = in[tc];            /* Input field at current tile */
      link *gtc = g[tc];              /* Gauge field at current tile */
      field *ptc = out[tc];           /* Output field at current tile */
      for(int z=0; z<L; z++) {
	int z0 = z*Sx*Sy;
	...
	for(int ty=1; ty<Sy-1; ty++) {
	  ...
	  for(int tx=1; tx<Sx-1; tx++) {
	     ...
	     *
	     */ 
	    p  = qtc[v00p].phi*gtc[v000].u[2] + qtc[v00m].phi*conj(gtc[v00m].u[2]); /* Direction x */
	    p += qtc[v0p0].phi*gtc[v000].u[1] + qtc[v0m0].phi*conj(gtc[v0m0].u[1]); /* Direction y */
	    p += qtc[vp00].phi*gtc[v000].u[0] + qtc[vm00].phi*conj(gtc[vm00].u[0]); /* Direction z */
	  ...
	  */
	  {
	    ...
	    int v0m0 = ym + z0;
	    int v00p = 1 + y0 + z0;
	    /* v00m is in another tile, with xt = Sx - 1 in that tile */
	    int v00m = Sx - 1 + y0 + z0;
	    _Complex float p;
	  ...
	  */
	  {
	    ...
	    int v0m0 = Sx-1 + ym + z0;
	    int v00m = Sx-2 + y0 + z0;
	    /* v00p is in another tile, with xt = 0 in that tile */
	    int v00p = 0 + y0 + z0;
	    _Complex float p;
	  ...
	  */
	  {
	    ...
	    int v00m = tx-1 + z0;
	    int v0p0 = tx + Sx + z0;
	    /* v0m0 is in another tile, with ty = Sy - 1 in that tile */
	  ...
	  */
	  {
	    ...
	    /* v0p0 is in another tile, with ty = 0 in that tile */
	    int v0p0 = tx + z0;
	    _Complex float p;
	    p  = qtc[v00p].phi*gtc[v000].u[2] + qtc[v00m].phi*conj(gtc[v00m].u[2]);
	...
	*/
	{
	  ...
	  int v0p0 = z0 + Sx;
	  int v00p = z0 + 1;
	  /* v0m0 and v00m are on neighboring tiles, with ty = Sy-1
	     and tx = Sx-1 resp.
	  */
	  int v00m = z0 + Sx-1;
	  int v0m0 = z0 + (Sy-1)*Sx;
	...
	*/
	{
	  ...
	  int v0p0 = z0 + Sx + Sx-1;
	  int v00m = z0 + Sx-2;
	  /* v0m0 and v00p are on neighboring tiles, with ty = Sy-1
	     and tx = 0 resp.
	  */
	  int v00p = z0;
	...
	*/
	{
	  ...
	  int v00p = z0 + y0 + 1;
	  int v0m0 = z0 + (Sy-2)*Sx;
	  /* v0p0 and v00m are on neighboring tiles, with ty = 0
	     and tx = Sx-1 resp.
	  */
	  int v0p0 = z0;
	  int v00m = z0 + y0 + Sx-1;
	...
	*/
	{
	  ...
	  int v00m = z0 + y0 + Sx-2;
	  /* v0p0 and v00p are on neighboring tiles, with ty = 0
	     and tx = 0 resp.
	  */
	  int v0p0 = z0 + Sx-1;
	  int v00p = z0 + y0;
