...
#pragma omp for    
  for(int z=0; z<L; z++) {
	 ...
	 * conj(g[vm00].u[0])
	 */
	p  = in[v00p].phi*g[v000].u[2] + in[v00m].phi*conj(g[v00m].u[2]); /* Direction x */
	p += in[v0p0].phi*g[v000].u[1] + in[v0m0].phi*conj(g[v0m0].u[1]); /* Direction y */
	p += in[vp00].phi*g[v000].u[0] + in[vm00].phi*conj(g[vm00].u[0]); /* Direction z */
